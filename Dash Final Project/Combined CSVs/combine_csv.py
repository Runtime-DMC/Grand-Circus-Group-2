...
columns_to_drop = {'index', 'description', 'date'}
...
for column in columns_to_merge:
    combined_data[column] = combined_data.apply(
        lambda row: row[column] if pd.isna(row['sal_' + column.split('_')[-1]]) else row['sal_' + column.split('_')[-1]],
        axis=1
    )
    combined_data.drop('sal_' + column.split('_')[-1], axis=1, inplace=True)

# Define the desired column order
column_order = [
    'title', 'company', 'skills', 'city', 'state', 'type', 'salary',
    'salary_type', 'salary_min', 'salary_max', 'salary_median', 'origin'
]
