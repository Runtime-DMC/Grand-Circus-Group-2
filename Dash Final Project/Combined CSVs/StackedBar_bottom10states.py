...
top_states = data.groupby('state')['title'].count().nsmallest(10).index.tolist()
...
fig = px.bar(stacked_data, x='state', y='count', color='title', barmode='stack', title='Top States with Least Jobs by Job Title')
